...
class EmployeesController extends \BaseController {
    ...
    if(Employee::where('employee.organization_id',Confide::user()->organization_id)->orderBy('id', 'DESC')->count() == 0){
      ...
      $pfn = preg_replace('/\D/', '', $pfn);
		...
		return View::make('employees.create', compact('currency','emps','citizenships','pfn','branches','departments','etypes','jgroups','banks','bbranches','educations'));
	}
  }
  ...
  public function serializeDoc(){
    ...
    $currency = Currency::whereNull('organization_id')->orWhere('organization_id',Confide::user()->organization_id)->first();
		return View::make('employees.edit', compact('currency','supervisor','count','subordinates','countk','countd','docs','kins','citizenships','contract','branches','educations','departments','etypes','jgroups','banks','bbranches','employee'));
	}
    ...
    $c = Supervisor::where('employee_id', $id)->count();
		...
		return View::make('employees.view', compact('employee','c','appraisals','kins','documents','occurences','properties','count','benefits'));
    ...
    $c = Supervisor::where('employee_id', $id)->count();
		...
		return View::make('employees.viewdeactive', compact('employee','c','appraisals','kins','documents','occurences','properties','count','benefits'));
